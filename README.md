...
> **電信陣營占台灣固網寬頻帳號數的比例，從 2019-01 的 75.37% 降到 2026-05 的 66.40%，
> 七年下降 8.97 個百分點。**
...
1. **契約驗證先於一切** — 欄位、列數、去除千分位逗號後是否為非負整數、小計算術、
   月份連續性、期間範圍。共 16 項；任一項不過，後面全部不跑。
...
| 查詢調校 | Parallel Seq Scan → Index Only Scan，buffers **16,604 → 409** |
...
建置過程中有六次，某個檢查宣稱的嚴謹度高於它實際做到的：容差內的案例被併進通過數；
註解寫著逐列比對、實際只比了總和；`EXPLAIN` 的 buffers 被跨節點重複加總，數字灌水六倍；
看板 hero 的數字寫成字面值，而這個專案的前提正是每個數字都由資料算出；
同一個寫死的數字十分鐘後又在第二個檔案犯一次；
把 Plotly 改成外部引用讓頁面變小、卻讓圖變成空白。

**六次都是在把產出擺到人眼前的那一刻才發現的**——渲染截圖、校驗碼、拿已知會失敗的輸入
重跑新檢查。CI 抓到第七次：契約還在要求它被寫下那天的快照，管線推進一個月就紅了。

結論不是「要更小心」，而是：**任何要進交付物的數字，都必須有一條獨立於產生它的程式碼的
驗證管道**。Ground Truth 自檢、逐列 MD5、渲染截圖回看、預先登記的日誌，都是為此而存在。

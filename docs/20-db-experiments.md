# 20 — 資料庫實驗（規格 5.4／第八節交付物 #3）

> **對應要求**：LA1 面試中主管提到「希望找**熟悉資料庫操作**以及懂 MLOps 的人」。
> 本文件回應前半句。後半句的誠實答法見 `../cht-projects/CLAUDE.md` 第三之二節。
...
規格 5.4 原本列了第三項：在 Teradata ClearScape（60 天免費）上做 PI 選對 vs 選錯的
資料傾斜對照。**依規格第十節的展示深度規則，這一項不放進交付物。**
...
我沒有 SQL 生產實務經驗，而**中華電信內部就是 Teradata 大戶**。
用一個註冊試用帳號、跑兩句 DDL 得到的「實驗」，等於**邀請全場最強的專家
往我最生的地方追問**。展示一項自己撐不到第三層的東西，代價高於不展示。
